    ...
    $(document).ready(function() {
        ...
        $('#addCondition').click(function() {
            ...
            if ( notAddConditionFlag ) {
                toastr.error('{{ __('querybuilder::messages.error_add_condition') }}.');
                return;
            }
            ...
            const newRow = `
                <div class="condition-card mb-2">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <input type="text" class="form-control condition-value" name="conditions[${conditionCount}][value]" placeholder="{{ __('querybuilder::messages.form_query_value_placeholder') }}" value="${qryValue ? qryValue : ''}">
                        </div>
                    </div>
                </div>
            `;
        ...
        // 📝 Updates the description (note) shown next to the selected operator in a condition card
        function updateOperatorNotes(selectElement) {
            let selectedOption = $(selectElement).find(":selected");

            // Fallback to 'Equal to' if no custom note is provided
            let notes = selectedOption.attr("data-notes") || "Equal to";

            // Display the note in the relevant part of the condition UI
